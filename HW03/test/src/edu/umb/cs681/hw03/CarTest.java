...
class CarTest {
	...
	void carCountTest() {
		...
		int actual = usedCars.stream()
			.reduce(0, (Integer result, Car car) -> ++result, (Integer finalResult, Integer intermediateResult) -> finalResult);
	...
	void carMinTest() {
		...
		int actual = usedCars.stream()
			...
			.reduce(Integer.MAX_VALUE, (Integer result, Integer carMileage) -> {
				if (carMileage < result) {
					return carMileage;
				}
				else {
					return result;
	...
	void carMaxTest() {
		...
		int actual = usedCars.stream()
			.map((Car car) -> car.getMileage())
			.reduce(0, (Integer result, Integer carMileage) -> {
				if (carMileage > result) {
					return carMileage;
				}
				else {
